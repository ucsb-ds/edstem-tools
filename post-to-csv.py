...
def edstem_post_to_csv(json_file, post_num):
    """
    The function takes as input the json file
    downloaded from the Threads JSON link
    on the page such as:
    https://edstem.org/us/courses/77825/analytics/discussion.

    If the function is called with
    edstem_post_to_csv("all-posts.json", 3)
    (assuming all-posts.json is in the same folder as the script),
    then the script will output two files:
    - comments_number_3.json: comments on that post in json form
    - comments_number_3.csv: commments on that post in CSV form

    The fields are:
    "name","email","created_at","text"
    """
    post_number = post_num # e.g., 3

    # Input and output file paths
    input_file = json_file # 'all-posts.json'
